...
<script lang="ts">
  ...
  const gltf_promise = useGltf('/spaceship/models/spaceship.glb');
  const map = useTexture('/spaceship/textures/energy-beam-opacity.png');
...
<T is={group} bind:ref dispose={false} {...rest}>
  <!-- {#await gltf_promise}
      {@render fallback?.()}
    {:then gltf} -->
  <svelte:boundary>
    {@const gltf = await gltf_promise}
    <T.Group
      scale={0.003}
