<script lang="ts">
  ...
  const map = useTexture('/spaceship/textures/star.png');
...
<svelte:boundary>
  <InstancedMesh limit={STARS_COUNT} range={STARS_COUNT}>
    ...
    <T.MeshBasicMaterial side={DoubleSide} alphaMap={await map} transparent />

    {#each stars as { id, position, length, color } (id)}
      <Instance {position} scale={[length, 1, 1]} {color} />
    {/each}
  </InstancedMesh>
  {#snippet pending()}
    <div>Loading...</div>
